#!/usr/bin/env bash
# =============================================================================
# LOCAL FORK MODIFICATION:
# Removed #I (window index/number) from window-status-format and
# window-status-current-format (lines ~319-330) to show only window names.
# Original: "#I #W" → Modified: "#W"
# =============================================================================
...
main() {
  ...
  *)
  ...
  # Window option (modified: removed #I to hide window numbers)
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[fg=${bg_bar},bg=${selection}]${left_sep}#[fg=${text},bg=${selection}] #W${current_flags} #[fg=${selection},bg=${bg_bar}]${left_sep}"
  else
    tmux set-window-option -g window-status-current-format "#[fg=${text},bg=${selection}] #W${current_flags} "
  fi
  ...
  tmux set-window-option -g window-status-format "#[fg=${text}]#[bg=${bg_bar}] #W${flags}"
